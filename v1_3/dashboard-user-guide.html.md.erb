...
##Platform
A platform configures platform specific attributes to send push messages. For example, this would include a certificate necessary to send messages to Apple's APNS, or a token necessary to send messages to Google's GCM. A platform has many devices.
...
For Android platforms you will need to provide **Project Number** and **Google Key** values.  The **Project Number** is
the numeric value found at the top middle of a project on the [Google Developers
Console](https://console.developers.google.com).  Do not use the 'Project ID'.  The **Google Key** is a Server API key,
created on the "Credentials" screen of the Google Developers Console.

For iOS platforms you will need to create a **APNS Development Certificate** and **APNS Production Certificate** using
the [Apple Developer Website](https://developer.apple.com).  These files, along with their associated private keys,
need to be exported from your **Keychain Access** program into a password protected **P12** file.  You will upload this
P12 file and provide its password when you create your platform on the PCF Push Notification Service dashboard.
